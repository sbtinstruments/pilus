...
def make_conc_plot():
    ...
    axs[0].plot(
        theory_conc_old, [a - tube_conc for a in measured_conc_old], "o", color="blue"
    )
    axs[0].plot(
        theory_conc_new, [a - tube_conc for a in measured_conc_new], "o", color="red"
    )
    axs[0].plot(
        np.linspace(0, max(theory_conc_old), 100),
        np.linspace(0, max(theory_conc_old), 100),
        "-",
        color="black",
    )
    axs[0].plot(
        np.linspace(0, max(theory_conc_old), 100),
        np.linspace(0, 1.3 * max(theory_conc_old), 100),
        "--",
        color="grey",
    )
    axs[0].plot(
        np.linspace(0, max(theory_conc_old), 100),
        np.linspace(0, 0.7 * max(theory_conc_old), 100),
        "--",
        color="grey",
    )
