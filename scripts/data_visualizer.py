...
transition_times = transition_times.rename(
    columns={"high_real.transition.offset": "high_real_transition_offset"}
)
...
color_scale = alt.Scale(domain=[6, 40], range=["#1FC3AA", "#8624F5"])


base = alt.Chart(transition_times).properties(width=500, height=500)
...
points = (
    ...
    .encode(
        ...
        color=alt.condition(
            selector, "pumpspeed:Q", alt.value("lightgray"), scale=color_scale
        ),
...
hists_data = (
    base.transform_filter(selector)
    .transform_joinaggregate(total="count(*)", groupby=["measurement"])
    .transform_calculate(pct="1 / datum.total")
    .mark_bar(opacity=0.5, thickness=100)
    .encode(
        alt.X(
            "high_real_transition_offset:Q",
            bin=alt.Bin(step=20000),
            scale=alt.Scale(domain=[160000, 800000]),
        ),
        alt.Y("sum(pct):Q", axis=alt.Axis(format="%")),
        color=alt.Color("pumpspeed:Q", scale=color_scale),
    )
)
...
final_chart = (points | concentrations | hists).properties(
    title=f"GES data: {ges_data} & lower_min: {lower_min}"
)
