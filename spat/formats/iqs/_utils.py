...
from typing import IO, DefaultDict, Iterable, Union
...
def _read_ihdr(f):
    ...
    for i in repeat(None, n_sites):  # pylint: disable=unused-variable
        ...
        for j in repeat(None, n_channels):  # pylint: disable=unused-variable
            ...
            ihdr[_site_name][_sample_name]["sample_max_signal_amplitude"] = read_int64(
                f
            )
            f.seek(f.tell() + 56)
    ...
    crc = read_uint32(
        f
    )  # TODO: move crc check into fnc decorator  #pylint: disable=unused-variable
    return ihdr
...
def _read_idat(f, ihdr: Union[dict, DefaultDict]):
    ...
    for site in ihdr.keys():
        for channel in ihdr[site].keys():
            ...
            idat_segment[site][channel]["re"] = np.array(
                [_read_int_func(f) for i in repeat(None, _n_samples)]
            )
            idat_segment[site][channel]["im"] = np.array(
                [_read_int_func(f) for i in repeat(None, _n_samples)]
            )
            ...
            try:
                for ch_key, channel in site.items():
                    for imre_key, imre_data in channel.items():
                        try:
                            data[site_key][ch_key][imre_key] += np.hstack(
...
def _add_time_vector(iqs: Union[dict, DefaultDict]):
    ...
    for site_name, site in iqs["data"].items():
        for channel_name, channel in site.items():
            ...
            channel["time"] = np.array(
                [(start_time + x * time_step) / 1e6 for x in range(0, n_data)]
            )
